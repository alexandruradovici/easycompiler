...
export class Rule extends ParseTree implements ParentNode
{
	...
	get (name:string, start:u32 = 0, stop: i32 = -1): ParseTree[]
	{
		let elementsList = this.children.filter (function (element, index)
		{
			return index >= start && (stop < 0 || index <= stop) && element.name === name;
		});
		return elementsList;
	}
